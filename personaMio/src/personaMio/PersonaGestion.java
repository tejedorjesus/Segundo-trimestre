...
public class PersonaGestion {
	...
	private static void generardatos(Persona[] personas) {
		...
		for (t=0;t<personas.length;t++){
			...
			personas[t] =new Persona(nombre[azar.nextInt(nombre.length)],
					azar.nextInt(10)+40,azar.nextInt(50)+150,azar.nextInt(99999999)+1);
		}
	}
	...
	private static void lista(Persona[] personas) {
		int t;
		for (t=0;t<personas.length;t++){
			System.out.printf(personas[t].getNombre() + "\t" +personas[t].getEdad()
					+ "\t"+personas[t].getAltura()+ "\t" +("%08d",personas[t].getDni()));
			System.out.println();
		}
	}
